...
async function syncDirectory(path: string): Promise<void> {
  ...
  try {
    ...
    // Root-owned Android ancestors (/data, /data/data) are unreadable to the app
    // user; the durability walk degrades there instead of failing publication.
  ...
  try {
    ...
    try {
      ...
      if (code === 'EACCES' || code === 'EPERM') {
        // Android SELinux forbids hard links; rename stays atomic and loses
        // only the first-writer-wins race single-process runs do not contend for.
...
async function publishStagedObject(
  ...
  try {
    ...
    // rename() consumed the staging name, so only a hard link leaves one.
    if (hardLinked) await unlink(staged.path)
