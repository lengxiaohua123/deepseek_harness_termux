...
if (process.platform !== 'linux' && process.platform !== 'darwin' && process.platform !== 'android') {
  ...
  throw new Error('build: system binaries are built on Linux, macOS, or Android; no native target for this host')
}
  ...
  for (const binary of spec.binaries) {
    ...
    if (binary.kind === 'static-musl' && process.platform === 'linux' && binary.tool === 'landlock-run') {
      ...
      if (process.platform === 'darwin') {
        ...
        if (binary.libc !== undefined && binary.libc !== 'glibc' && binary.libc !== 'musl') {
