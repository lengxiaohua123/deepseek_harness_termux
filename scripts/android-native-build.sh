...
# patched store copy.
KOFFI_MOD="$(ls -d "$ROOT"/node_modules/.pnpm/koffi@*_patch_hash=*/node_modules/koffi 2>/dev/null | head -1 || true)"
if [ -n "$KOFFI_MOD" ]; then
...
NPTY_MOD="$(ls -d "$ROOT"/node_modules/.pnpm/node-pty@*/node_modules/node-pty | head -1)"
