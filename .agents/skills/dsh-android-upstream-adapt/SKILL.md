...
Full-suite baseline after the 0.1.5-rc.2 sync (1541s / 25.7 min, `--maxWorkers=4`, node v26.4.0 on 8 cores): 22365 passed / 109 failed / 1 expected fail / 133 skipped (22614 tests, 1276 files). Every one of the 109 is accounted for, and two of the groups below are since fixed — a rerun should read 22379 passed / 95 failed, a delta verified by rerunning the two affected files rather than by a second full pass:

- **63 `experimental/code-runtime-python/tests/runtime.spec.ts` — bionic memory-budget semantics.** A second run on an otherwise idle machine reproduced all of them, so this is not machine load. The affected cases drive `RLIMIT_AS` and large allocations, and on Android they surface as CPython `MemoryError`, `SIGABRT` worker exits (`code=null, signal=SIGABRT`), and limit-kind mismatches (`exception` where glibc yields `output-limit`, `worker-exit` where it yields `timeout`). The product path therefore degrades from a classified budget outcome to an abort on Termux. Experimental package; the tests assert the glibc shape, so adapting them would mean rewriting what they verify. (A further 2 cases in this file were only missing `which`; `pkg install which` cleared them, 65 → 63.)
- **12 `spill/spill-local` — a real gap, since fixed (see the Adaptation ledger).** `hasProtectedAncestors` refused any root with a group-writable, non-sticky ancestor, and `/data` plus `/data/data` are `771` owned by the system uid, so every root was rejected, the startup sweep was skipped, and spilled session data was never reclaimed. Proven by calling `sweepSpillRoots` directly with a warning sink: `skipped unsafe root …`. The sweep now treats the Android tree as OS-protected, and the file reruns 43/43.
...
Green in this run, so a failure there is a real regression: `process-exit`, `acp-snapshot`, `session-query-sqlite`, `session-persistence-sqlite`, `tool-bash`; and `spill-local` after the fix above.
...
| `packages/spill/spill-local/src/cleanup.ts` | `hasProtectedAncestors` returns true on Android (`android` platform, or `PREFIX` naming Termux) instead of testing ancestor write classes | Android's `/data` and `/data/data` are `771` owned by the system uid, so the write-class test rejected every root; the sweep then never ran and spilled session data was never reclaimed. There the OS sandbox (uid isolation plus SELinux), not the mode bits, is the boundary — the same reason the file already returns true for `win32` |
...
| `spill-local` cleanup tests fail with `expected true to be false` and no error | The sweep is refusing the root: it logs nothing by default, so call `sweepSpillRoots` with a warning sink and look for `skipped unsafe root`. Fixed on Android by the `cleanup.ts` ledger row; a recurrence means upstream rewrote that file and dropped the early return |
